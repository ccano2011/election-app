...
app.get('/', (req, res) => res.send("Elections Back-end Root; Test CRUD in postman with localhost:3000/ballotreq or localhost:3000/users"))
...
app.put('/user/:id', async (req, res) => {
    const { id } = req.params
    await userSchema.findByIdAndUpdate(id, req.body, { new: true }, (error, info) => {
        if (error) {
            return res.status(500).json({ error: error.message })
        }
        if (!info) {
